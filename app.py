...
def index():
    ...
    if request.method == 'POST':
        ...
        # Check for both 'price' and 'rating' in tokens
            ...
            # Find the best 5 matches using fuzzy string matching
            matches = process.extract(query, df1['Name'], limit=10)

            # Check if any match is above a certain threshold (adjust as needed)
            threshold = 30
            valid_matches = [match for match in matches if match[1] >= threshold]

            if valid_matches:
                # Get the matched phone names
                matched_phone_names = [match[0] for match in valid_matches]

                # Retrieve the corresponding rows from the DataFrame
                matched_rows = df1[df1['Name'].isin(matched_phone_names)]

                # Display the results
                chatbot_response = "TOP 5 MATCHES ARE:\n\n"
                for _, row in matched_rows.iterrows():
                    chatbot_response += f"{row.to_string(header=False)}\n\n"
                
                chatbot_response = '<br><br>'.join(chatbot_response.split('\n'))
            else:
                chatbot_response = "Couldn't find a match."
